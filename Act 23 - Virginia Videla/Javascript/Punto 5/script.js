...
function atender_siguiente(cola_espera)
{
    ...
    for (let i = 0; i < length(cola_espera); i++)
    {
        if (cola_espera[i][1] > cola_espera [indice][1])
        {
            indice = i;
            let paciente = cola_espera.pop(indice);
            
            return ["Se atiende a", paciente[0], "Nivel", paciente[1]]
        }
    }
}
...
console.log("Cola restante:" + cola);
