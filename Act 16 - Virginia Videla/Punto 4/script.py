...
for x in range(6):
    nombre = input(f"Escriba el nombre del profesor {x+1}: ")
    puntaje = int(input(f"Escriba el puntaje de {nombre}: "))
...
for x in range(6):
    for k in range(x + 1, 6):
        ...
        if puntajes[x] < puntajes[k]:

            auxpuntaje = puntajes[x]
            puntajes[x] = puntajes[k]
            puntajes[k] = auxpuntaje
